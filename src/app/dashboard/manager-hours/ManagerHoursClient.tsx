...
export default function ManagerHoursClient({ hours: initial, managerId, managerName, branchId, isSuperAdmin, allManagers }: Props) {
  ...
  function handleExcelExport() {
    const rows = filtered.map(h => ({
      Manager: h.managers?.name ?? managerName,
      Branch: h.managers?.branches?.name ?? '',
      Date: h.date,
      Hours: h.hours,
      Notes: h.notes ?? '',
    }))
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Manager Hours')
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-gray-800">Manager Hours</h2>
      ...
      {isSuperAdmin ? (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
          <div className="bg-blue-50 rounded-xl p-4">
            ...
            <p className="text-sm text-blue-600 font-medium mt-1">{filterManager ? 'Hours (filtered)' : 'Total Hours'}</p>
          </div>
          {managerTotals.slice(0, 3).map(m => (
            <div key={m.name} className="bg-gray-50 rounded-xl p-4">
              <p className="text-2xl font-bold text-gray-700">{m.total.toFixed(1)}h</p>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          <div className="bg-blue-50 rounded-xl p-4">
            <p className="text-2xl font-bold text-blue-700">{totalHours.toFixed(1)}</p>
            <p className="text-sm text-blue-600 font-medium mt-1">Your Total Hours</p>
          </div>
          <div className="bg-green-50 rounded-xl p-4">
            <p className="text-2xl font-bold text-green-700">{hours.length}</p>
            <p className="text-sm text-green-600 font-medium mt-1">Entries</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              {[...(isSuperAdmin ? ['Manager', 'Branch'] : []), 'Date', 'Hours', 'Notes', ''].map(h => (
          ...
          <tbody className="divide-y divide-gray-100">
            {filtered.map(h => (
              <tr key={h.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 font-medium text-gray-800">{h.hours}h</td>
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={isSuperAdmin ? 6 : 4} className="px-4 py-8 text-center text-gray-400">No entries yet</td></tr>
            )}
          </tbody>
        </table>
      </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-md">
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-bold text-lg">{editing ? 'Edit Entry' : 'Add Hours'}</h3>
              <button onClick={() => setShowForm(false)}><X size={20} /></button>
            </div>
            <p className="text-sm text-gray-500 mb-4">Logging hours for: <span className="font-medium text-gray-700">{managerName}</span></p>
